...
node_ids = node.id;
connectivity_matrix = sparse(length(node_ids));
for curway=1:ways_num
    ...
    for othernode_local_index=2:nodes_num
        ...
        for otherway=1:ways_num
            ...
            if isempty(idx) == 0
                Nsamends = Nsamends +1;
                [key, val] = get_way_tag_key(way.tag{1, curway}, 'maxspeed');
                if strcmp(key, 'maxspeed') == 0
                    speed = 0;
                else
                    pat = digitsPattern;
                    speed_str = extract(val,pat);
                    if isempty(speed_str)
                        speed = 0;
                    else
                        speed = str2double(speed_str{1});
                    end
                end
                connectivity_matrix(node1_index, othernode_index) = speed;
                [key, val] = get_way_tag_key(way.tag{1, curway}, 'oneway');
                if strcmp(key, 'oneway') == 0 || (strcmp(key, 'oneway') && strcmp(val, 'no'))
                    connectivity_matrix(othernode_index, node1_index) = speed;
                end
                node1_index = othernode_index;
